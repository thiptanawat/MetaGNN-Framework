#!/usr/bin/env python3
"""Degree-preserving rewiring null for the double hold-out.

Identical to double_holdout_ctrl.py except that the network the graph model passes
messages over is replaced by a random graph with exactly the same degree sequence.
For each edge type the source endpoints are held fixed and the target endpoints are
permuted, which is the configuration model: every node keeps its degree in every
relation, and nothing else about the network survives.

The question it answers is the one a reader asks after seeing that a structure-only
model beats an expression-based one. Is the useful structure the biological
organization of the network, or is it generic graph statistics that any graph with
this degree sequence would supply? If accuracy holds up under rewiring, the graph
model is reading degree. If it falls, the model is reading the metabolic network.

Everything else, including the folds, the seeds, the architecture and the feature
substitution modes, is unchanged, so a rewired cell is directly comparable to the
cell of the same name in the main grid.
...
ap.add_argument('--feature_mode', choices=['real','mean','zero'], default='real',
...
ap.add_argument('--rewire', action='store_true',
                help='replace every edge index with a degree-preserving random rewiring '
                     '(configuration model: source endpoints fixed, target endpoints permuted)')
ap.add_argument('--rewire_seed', type=int, default=7,
                help='seed for the rewiring, separate from the training seed so the same '
                     'split can be run against several rewirings')
...
EDGE_OVERRIDE = None   # {relation: edge_index} for the rewired network, or None for the real one

def rewire_edges(edge_index, rng):
    """Configuration-model rewiring: keep sources, permute targets.

    Permuting the target row leaves the multiset of target endpoints unchanged, so
    every target node keeps exactly its original degree, and the source row is
    untouched, so every source node does too. Multi-edges and self-loops can appear,
    as they do in any configuration model.
    """
    e = edge_index.clone()
    perm = torch.from_numpy(rng.permutation(e.shape[1]))
    e[1] = e[1][perm]
    return e

def build_rewiring(sample, seed):
    rng = np.random.default_rng(seed)
    out = {}
    for r in sample.edge_types:
        out[r] = rewire_edges(sample[r].edge_index, rng)
    return out
...
def fwd(model, b):
    ...
    if EDGE_OVERRIDE is not None:
        ei = {r: EDGE_OVERRIDE[r].to(xr.device) for r in b.edge_types}
    else:
        ei = {r: b[r].edge_index for r in b.edge_types}
    return model(x_dict={'reaction': xr, 'metabolite': b['metabolite'].x}, edge_index_dict=ei)
...
if A.rewire:
    EDGE_OVERRIDE = build_rewiring(probe[0], A.rewire_seed)
    for _r, _e in EDGE_OVERRIDE.items():
        _real = probe[0][_r].edge_index
        _d_real = torch.bincount(_real[1], minlength=int(_real[1].max()) + 1)
        _d_new = torch.bincount(_e[1], minlength=int(_real[1].max()) + 1)
        _same = bool(torch.equal(_d_real.sort().values, _d_new.sort().values))
        _kept = int((_real[1] == _e[1]).sum())
        print(f'rewired {_r}: {_e.shape[1]:,} edges | target degree sequence preserved: {_same} '
              f'| endpoints unchanged by chance: {_kept} ({100*_kept/_e.shape[1]:.3f}%)', flush=True)
...
for spec in A.models:
    ...
    for pf, sp in enumerate(pfolds):
        ...
        elif A.feature_mode == 'mean':
            import h5py as _h5
            acc = np.zeros(N_RXN)
            for _p in sp['train']:
                with _h5.File(os.path.join(FEAT, _p + '.h5')) as _h: acc += _h['X_R'][:, 0]
        ...
        for rf, (rtr, rte) in enumerate(rfolds):
            ...
            tag = f'{name}_mb{lam}_p{pf}_r{rf}' + ('' if A.feature_mode=='real' else f'_{A.feature_mode}') \
                  + ('' if not A.rewire else f'_rewire{A.rewire_seed}'); rfile = os.path.join(A.out, tag + '.json')
            ...
            res = dict(model=name, lambda_mb=lam, pfold=pf, rfold=rf, n_train=len(sp['train']), n_test=len(sp['test']), n_heldout_rxn=int(mt.sum()), epochs=ep, best_val=round(best, 4), minutes=round((time.time() - t0) / 60, 1),
                       ...
                       rewired=bool(A.rewire), rewire_seed=(A.rewire_seed if A.rewire else None))
