"""Authors' scripts/model/pred_enzyme.py, copied verbatim except for three changes, all marked
PATCH: (1) torch.set_grad_enabled(False) after model.eval() -- the original keeps the autograd
graph for every node and exhausts memory; (2) cell_net.py is imported from the DeepMeta clone
named in config.py and train_genes.csv is read from there, instead of relying on the current
working directory; (3) nothing else. Inference is unchanged.

Requires TORCH_FORCE_NO_WEIGHTS_ONLY_LOAD=1 (torch>=2.6 refuses to unpickle the PyG Data
objects and the published checkpoint otherwise) and -a cpu on a machine without CUDA.
"""
import numpy as np
import pandas as pd
import torch
...
import config as _C          # noqa: E402  (audit config: paths only)
sys.path.insert(0, _C.CELL_NET_DIR)
...
train_genes = pd.read_csv(_C.DEEPMETA_FILES["train_genes.csv"])
...
model.eval()
torch.set_grad_enabled(False)  # PATCH: inference without autograd (original script keeps the autograd graph)
