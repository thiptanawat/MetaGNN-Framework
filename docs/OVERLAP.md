...
companion preprint" in Reaction-Scoring Audit, "Relation to the companion audit" in Language-Model Evidence) and in their
...
1. **The companion preprint.** An external document by the same authors, cited as
   ...
   place, fixed the graph neural network configuration the Reaction-Scoring Audit studies, and separately
...
2. **the Reaction-Scoring Audit** ("Holding out reactions as well as patients changes what transcriptome-based
   metabolic reaction scoring benchmarks measure"), which the Language-Model Evidence's bibliography cites as
   `\cite{CompanionDH}`, correctly, under its current title.
3. **the Language-Model Evidence** ("Response sensitivity does not establish patient-specific validity in
   language-model annotation of metabolic reactions"), which the Reaction-Scoring Audit's bibliography cites as
   `\cite{CompanionLLM}`. **This citation is stale**: the Reaction-Scoring Audit's bibliography entry for
   ...
   it," which is an earlier title of Language-Model Evidence and does not match the Language-Model Evidence's current title
   above. The reverse citation, the Language-Model Evidence quoting the Reaction-Scoring Audit's title, is currently correct. This
...
Reaction-Scoring Audit states the graph configuration is "carried forward unchanged from our companion
preprint" and describes the activity-label union as "our construction rather than a released
artifact, shared with the companion preprint." the Language-Model Evidence states that it "uses the cohort, the
...
the same repository," referring there to Reaction-Scoring Audit, not to the external preprint. The word
...
- **624 patients** (`\nPat` in Reaction-Scoring Audit, `\netPat` in Language-Model Evidence), **10,600 reactions**
  ...
  active** under the label union (`\nActive`/`\netActive`, base rate 0.3188 in the Reaction-Scoring Audit's
  four-decimal form and 0.319 in the Language-Model Evidence's three-decimal form: the same value, not a
  discrepancy).
- **The raw-expression baseline, 0.6342**, and **the patient-invariant availability
  ...
  respectively. The Reaction-Scoring Audit names these `\rawExpr` and `\indicator`; the Language-Model Evidence names the same two
...
- **One genuine minor inconsistency**: the share of expression-bearing reactions that are
  labeled active is quoted as 40.7% in Reaction-Scoring Audit (`\prevExpr`) and 40.8% in Language-Model Evidence
...
- **The companion preprint's own numbers**, quoted rather than recomputed: an AUROC of
  ...
  originally archived models had received no patient input. The Language-Model Evidence's introduction and
  discussion both quote all three figures; the Reaction-Scoring Audit does not restate them numerically but
  points to the same preprint for the diagnostics behind its own ceiling argument.

## Cohort, labels, model and what is new

| | Companion preprint (external, cited only) | Reaction-Scoring Audit | Language-Model Evidence |
|---|---|---|---|
| **Cohort** | Curated the colorectal cohort used by both manuscripts here, and a separate breast cancer cohort used only in the companion preprint itself | TCGA-COAD and TCGA-READ primary tumors from the GDC, 624 patients after deduplication (the same cohort; the Reaction-Scoring Audit's Data availability section gives a GDC manifest retrieval date of 9 March 2026) | The identical 624-patient cohort, described in the same terms; the Language-Model Evidence additionally uses the patients' microsatellite instability status for its one patient-varying target |
...
Everything in the Reaction-Scoring Audit's list is explicitly stated, in its own introduction, to be absent
from the companion preprint ("Everything this paper is about is absent from it"). Everything
in the Language-Model Evidence's list follows the same pattern relative to both the companion preprint and Paper
1: The Language-Model Evidence does not repeat the Reaction-Scoring Audit's double hold-out, naive baselines or phenotype probes,
and Reaction-Scoring Audit does not repeat any of the Language-Model Evidence's language-model experiments. The two manuscripts'
