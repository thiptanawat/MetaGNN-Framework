"""
MetaGNN Training on TCGA-BRCA
==============================
Runs MetaGNN (GATv2 heterogeneous graph attention) on breast cancer data.
...
Usage:
  ...
  python 03_train_metagnn_brca.py \
      --data_dir ./data_brca/processed/ \
      --config benchmark_220 \
      --n_patients 220 \
      --output_dir ./results_brca/benchmark_220/

  # Config B: Full-cohort 5-fold CV
  python 03_train_metagnn_brca.py \
      --data_dir ./data_brca/processed/ \
      --config full_cohort \
      --output_dir ./results_brca/full_cohort/
...
class MetaGNN(torch.nn.Module):
    ...
    Identical to the CRC version — no architecture changes for BRCA.
    """
    def __init__(
        self,
        rxn_in_dim: int = 2,
...
CONFIGS = {
    'benchmark_220': {
        ...
        'dropout': 0.2,
        'lambda_mb': 0.2,
        # n_patients omitted → uses ALL available patients (1,095 for BRCA)
        # Override with --n_patients 220 for direct CRC-comparable benchmark
        'split_ratio': [0.70, 0.15, 0.15],
        'description': 'Benchmark (bipartite-only, 143K params)',
    },
    'full_cohort': {
        'hidden_dim': 256,
        'n_layers': 3,
        ...
        'lambda_mb': 0.2,
        'n_folds': 5,
        'description': 'Full-cohort 5-fold CV (expanded mode, 9.67M params)',
    },
}
...
# ═════════════════════════════════════════════════════════════════════════════
def main():
    parser = argparse.ArgumentParser(description="Train MetaGNN on TCGA-BRCA")
    parser.add_argument("--data_dir", required=True, help="Processed BRCA data directory")
    parser.add_argument("--config", choices=list(CONFIGS.keys()), default="benchmark_220")
    parser.add_argument("--output_dir", default="./results_brca/")
    ...
    results_path = os.path.join(args.output_dir, 'results_brca.json')
    ...
    logger.info("=" * 60)
    logger.info("MetaGNN-BRCA TEST RESULTS")
    logger.info("=" * 60)
    logger.info(f"  Cancer type:  BRCA (Breast Invasive Carcinoma)")
