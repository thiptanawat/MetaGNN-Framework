"""
TCGA-BRCA RNA-seq Data Acquisition Pipeline for MetaGNN
========================================================
Downloads STAR gene counts from the GDC Data Portal for TCGA-BRCA
(Breast Invasive Carcinoma) using the GDC REST API.
...
Data source:
  TCGA-BRCA via GDC Data Portal (https://portal.gdc.cancer.gov/)
...
Expected yield: ~1,098 unique patients (primary tumour, 01A samples)

Usage:
  python 01_download_tcga_brca.py --output_dir ./data_brca/raw
...
# ─────────────────────────────────────────────────────────────────────────────
# Step 1: Query GDC for TCGA-BRCA STAR gene count file UUIDs
# ─────────────────────────────────────────────────────────────────────────────
def query_gdc_brca_files(sample_type: str = "Primary Tumor") -> pd.DataFrame:
    """
    Query the GDC API for all STAR gene count files in TCGA-BRCA.
    ...
    logger.info("Querying GDC API for TCGA-BRCA STAR gene count files...")
...
# ─────────────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Download TCGA-BRCA RNA-seq for MetaGNN")
    parser.add_argument("--output_dir", default="./data_brca/raw",
                        help="Output directory for downloaded files")
    parser.add_argument("--merged_output", default="./data_brca/tcga_brca_tpm_log2.tsv",
                        help="Path for merged gene × patient matrix")
    parser.add_argument("--manifest_output", default="./data_brca/gdc_manifest_brca.tsv",
    ...
    if not args.skip_download:
        ...
        manifest_df = query_gdc_brca_files(sample_type="Primary Tumor")
