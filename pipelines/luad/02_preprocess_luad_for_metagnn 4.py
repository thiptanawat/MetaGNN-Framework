"""
MetaGNN-LUAD Feature Engineering Pipeline
==========================================
Converts TCGA-LUAD RNA-seq TPM matrix into MetaGNN-compatible
...
Inputs:
  - tcga_luad_tpm_log2.tsv  (from 01_download_tcga_luad.py)
...
Outputs:
  - data_luad/processed/reaction_features/<TCGA-barcode>.h5  per patient
  - data_luad/processed/activity_pseudolabels.pt  (expression-thresholded)
  - data_luad/processed/edge_indices/  (symlinked from CRC)
  - data_luad/processed/metabolite_features.h5  (symlinked from CRC)
...
def build_gene_id_mapping(
    ...
    # Load the mapping saved by 01_download_tcga_luad.py
    ensembl_to_symbol = {}
...
def save_processed_data(
    ...
    """
    Save LUAD data in the exact same format as MetaGNN-CRC/code/processed_690/.
...
def main():
    parser = argparse.ArgumentParser(
        description="Preprocess TCGA-LUAD for MetaGNN (same format as CRC)"
    )
    parser.add_argument("--tpm_matrix", required=True,
                        help="Path to merged LUAD log2(TPM+1) matrix TSV")
    parser.add_argument("--crc_processed", required=True,
                        help="Path to MetaGNN-CRC/code/processed_690/")
    parser.add_argument("--output_dir", default="./data_luad/processed/",
                        help="Output directory for processed LUAD data")
    ...
    logger.info("=" * 60)
    logger.info("LUAD preprocessing complete!")
    ...
    logger.info("=" * 60)
    logger.info("Next: Run MetaGNN training with the LUAD data:")
    logger.info("  python 03_train_metagnn_luad.py --data_dir ./data_luad/processed/")
